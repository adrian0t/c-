...
namespace lab3.Models
{
    public class Dane
    {
        ...
        [DataType(DataType.Password)] public string Hasło { get; set; }

        [Required(ErrorMessage = "Proszę potwierdzić hasło")]
        [Compare("Hasło", ErrorMessage = "Hasła muszą się zgadzać")]
        [DataType(DataType.Password)]
        public string PotwierdzenieHasła { get; set; }

        [Required(ErrorMessage = "Nr telefonu jest wymagany")]
        [RegularExpression(@"^\d{9}$", ErrorMessage = "Nr telefonu musi mieć dokładnie 9 cyfr")]
